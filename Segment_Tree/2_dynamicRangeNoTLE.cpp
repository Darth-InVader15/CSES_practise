...
void build(int ind, int l, int r, vector<int> &arr) {
    ...
    tree[ind] = (tree[2 * ind + 1] + tree[2 * ind + 2]); // minima
}

void update(int ind, int l, int r, int pos, int val) {
    if (l == r) {
        tree[ind] = val; // values updated
        return;
    }
    int mid = (l + r) / 2;
    if (pos <= mid)     //for reducing extra recursions
    {
...
ll query(int ind, int l, int r, int lq, int rq) {
    if (lq > r || rq < l) return 0; // no overlap
    if (lq <= l && r <= rq) return tree[ind]; // complete overlap 
    int mid = (l + r) / 2;
    ll lt = query(2 * ind + 1, l, mid, lq, rq);
    ll rt = query(2 * ind + 2, mid + 1, r, lq, rq);
    return (lt + rt);
}
...
void solve() {
    ...
    while (q--) {
        ...
        if (type == 1) {
            ...
            cout << (ll)query(0, 0, n - 1, x - 1, y - 1) << endl;
        }
    }
}
